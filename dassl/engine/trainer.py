...
class SimpleTrainer(TrainerBase):
    ...
    def after_epoch(self):
        last_epoch = (self.epoch + 1) == self.max_epoch  # 判断是否是最后一个 epoch
        do_test = not self.cfg.TEST.NO_TEST  # 是否进行测试
        meet_checkpoint_freq = (
            (self.epoch + 1) % self.cfg.TRAIN.CHECKPOINT_FREQ == 0
            if self.cfg.TRAIN.CHECKPOINT_FREQ > 0 else False
        )  # 是否满足保存 checkpoint 的频率

        # 如果是最后一个 epoch 或者需要进行测试
        if do_test and self.cfg.TEST.FINAL_MODEL == "best_val":
            curr_result = self.test(split="val")  # 在验证集上测试
            is_best = curr_result > self.best_result  # 如果当前结果是最好的，保存模型
            if is_best:
    ...
    def run_epoch(self):
        ...
        for self.batch_idx in range(self.num_batches):
            ...
            if meet_freq or only_few_batches:
                ...
                info = []
                info += [f"训练轮次 [{self.epoch + 1}/{self.max_epoch}]"]
                info += [f"批次索引 [{self.batch_idx + 1}/{self.num_batches}]"]
                info += [f"训练时间 {batch_time.val:.3f} ({batch_time.avg:.3f})"]
                info += [f"加载时间 {data_time.val:.3f} ({data_time.avg:.3f})"]
                info += [f"{losses}"]
                info += [f" 学习率 {self.get_current_lr():.4e}"]
                info += [f" 预计剩余时间 {eta}"]
                print(" ".join(info))
...
class TrainerX(SimpleTrainer):
    ...
    def run_epoch(self):
        ...
        losses = MetricMeter()  # 记录损失的类
        batch_time = AverageMeter()  # 记录每个 batch 的时间
        data_time = AverageMeter()  # 记录数据加载的时间
        self.num_batches = len(self.train_loader_x)  # 获取当前训练集的 batch 数量

        end = time.time()  # 获取当前时间戳，方便后续计算时间

        for self.batch_idx, batch in enumerate(self.train_loader_x):
            data_time.update(time.time() - end)  # 更新数据加载时间
            loss_summary = self.forward_backward(batch)  # 前向传播与反向传播，计算损失并优化
            batch_time.update(time.time() - end)  # 更新当前 batch 的训练时间
            losses.update(loss_summary)  # 更新损失

            # 每隔一段时间输出一次训练进度
            meet_freq = (self.batch_idx + 1) % (self.cfg.TRAIN.PRINT_FREQ * 1) == 0  # Double the print interval
            only_few_batches = self.num_batches < self.cfg.TRAIN.PRINT_FREQ
            if meet_freq or only_few_batches:
                nb_remain = 0
                nb_remain += self.num_batches - self.batch_idx - 1
                ...
                eta_seconds = batch_time.avg * nb_remain
                eta = str(datetime.timedelta(seconds=int(eta_seconds)))  # 计算剩余时间
            ...
            n_iter = self.epoch * self.num_batches + self.batch_idx  # 计算当前迭代次数
            for name, meter in losses.meters.items():
                self.write_scalar("train/" + name, meter.avg, n_iter)  # 记录损失
            self.write_scalar("train/lr", self.get_current_lr(), n_iter)  # 记录学习率

            end = time.time()  # 更新当前时间
        return losses  # 返回一个包含当前 epoch 损失信息的对象
